...
class OpenRouterClient(LLMClient):
    def __init__(self, api_key: str):
        self.client = AsyncOpenAI(
            ...
            # Explicit bounded timeout (matches DeepSeek): a slow/hung provider
            # must error and close the SSE stream instead of leaving the chat
            # "loading" for the SDK's ~10-minute default.
            timeout=httpx.Timeout(90.0, connect=15.0),
        )

    async def chat(self, messages: list[dict], tools: list[dict] | None = None, **overrides) -> dict:
        body = dict(model=overrides.get("model") or DEFAULT_MODEL, messages=messages)
        if tools:
            body["tools"] = tools
    ...
    async def stream_chat(self, messages: list[dict], tools: list[dict] | None = None) -> AsyncIterator[str]:
        kwargs = dict(model=DEFAULT_MODEL, messages=messages, stream=True)
        if tools:
            kwargs["tools"] = tools
