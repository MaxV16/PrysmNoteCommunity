...
async def list_links(
    ...
    if task_id:
        stmt = stmt.where(
            (TaskLink.source_task_id == require_uuid(task_id)) | (TaskLink.target_task_id == require_uuid(task_id))
        )
...
async def create_link_route(
    ...
    existing = await session.execute(
        select(TaskLink).where(
            ...
            TaskLink.source_task_id == require_uuid(request.source_task_id),
            TaskLink.target_task_id == require_uuid(request.target_task_id),
        )
    )
    ...
    link = TaskLink(
        user_id=user.id,
        source_task_id=require_uuid(request.source_task_id),
        target_task_id=require_uuid(request.target_task_id),
...
async def delete_link_route(
    ...
    result = await session.execute(
        select(TaskLink).where(TaskLink.id == require_uuid(link_id), TaskLink.user_id == user.id)
    )
