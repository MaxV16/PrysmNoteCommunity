...
// Validate and coerce an imported backup's tasks so arbitrary JSON can't inject
// unknown objects/fields into the store. Returns null when the payload is not a
// valid tasks list; otherwise returns only well-formed tasks.
function sanitizeImportedTasks(value: unknown): Task[] | null {
  ...
  for (const item of value) {
    ...
    const task: Record<string, unknown> = { id, title };
    for (const key of IMPORT_TASK_FIELDS) {
      if (key in rec) task[key] = rec[key];
    }
    out.push(task as unknown as Task);
  }
...
export default function SettingsPage() {
  ...
  // per-session encryption key). Only the caller's own stored keys are returned.
  useEffect(() => {
    if (!user || keys.length === 0) return;
    (async () => {
      for (const k of keys) {
        const local = await getApiLocalKey(k.provider);
        if (!local) {
          await recoverKey(k.provider);
        }
      }
    })();
  }, [user, keys, recoverKey, getApiLocalKey]);
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const tasks = sanitizeImportedTasks(data?.tasks);
        if (!tasks) {
          alert("Invalid backup file format.");
          return;
        }
        if (!window.confirm(`Import ${tasks.length} tasks? This will overwrite current data.`)) return;
            ...
            <section className="space-y-5">
              ...
              <div className="card p-6 space-y-4">
                ...
                <div className="rounded-xl bg-elevated px-4 py-3 border border-border space-y-1 font-mono text-xs text-secondary">
                  <div className="flex justify-between"><span className="text-muted">API Version</span><span>{process.env.NEXT_PUBLIC_GIT_SHA ? process.env.NEXT_PUBLIC_GIT_SHA.slice(0, 7) : "development"}</span></div>
            ...
            <section className="card p-6 space-y-5">
              ...
              <div className="space-y-3">
                <div className="rounded-xl bg-elevated px-4 py-3 border border-border flex justify-between">
                  <span className="text-sm text-secondary">Version</span><span className="text-sm font-mono text-primary">{process.env.NEXT_PUBLIC_GIT_SHA ? process.env.NEXT_PUBLIC_GIT_SHA.slice(0, 7) : "development"}</span>
                </div>
                <div className="rounded-xl bg-elevated px-4 py-3 border border-border flex justify-between">
                  <span className="text-sm text-secondary">License</span><span className="text-sm text-primary">AGPL-3.0 (Community)</span>
                </div>
                <div className="rounded-xl bg-elevated px-4 py-3 border border-border flex justify-between">
                  <span className="text-sm text-secondary">Build</span><span className="text-xs font-mono text-muted">{process.env.NEXT_PUBLIC_GIT_SHA || process.env.NODE_ENV || "development"}</span>
                </div>
              </div>
