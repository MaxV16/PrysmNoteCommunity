...
function rememberUser(userId: string | undefined): void {
  try {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    // Account change: only wipe local state (incl. API keys) if this login is for
    // a DIFFERENT account than the one stored. Re-login to the same account keeps
    // the user's cached provider keys.
