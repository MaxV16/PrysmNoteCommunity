 ...
 *   3. Create a task with start_date = today (local).
...
async function main() {
  ...
  let cookie = extractAccessCookie(reg.cookie);
  assert(cookie, "register set an access_token cookie");
  ...
  cookie = extractAccessCookie(login.cookie) || cookie;
  assert(cookie, "login set an access_token cookie");
  ...
  const fetched = await request(`/tasks/${createdBody.id}`, {}, cookie);
  ...
  await request(`/tasks/${createdBody.id}`, { method: "DELETE" }, cookie);
